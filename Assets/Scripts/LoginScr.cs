...
public class LoginScr : mScreen, IActionListener
{
    ...
    public LoginScr()
    {
        ...
        tfUser.x = xLog + 10;
        tfUser.y = yLog + 20;
        cmdOK = new(mResources.OK, this, 2008, null);
        cmdOK.x = GameCanvas.w / 2 - 84;
        cmdOK.y = cmdLogin.y;
        ModFunc.cmdAccManager = new(ModFunc.strAccManager, ModFunc.GI(), 101, null);
        ...
        ModFunc.cmdCloseAccManager = new(mResources.CLOSE, ModFunc.GI(), 104, null);
    ...
    public override void paint(mGraphics g)
    {
        ...
        mFont.tahoma_7_white.drawStringBorder(g, "v" + GameMidlet.VERSION, GameCanvas.w - 2, 17, 1, mFont.tahoma_7_grey);
        ...
        if (GameCanvas.currentDialog == null)
        {
            if (ModFunc.isOpenAccMAnager)
            {
                ...
                PopUp.paintPopUp(g, xPop, yPop, w, h, -1, isButton: true);
                left = null;
                center = null;
                ...
                if (GameCanvas.h > 160 && ModFunc.imgLogoBig != null)
                {
                    ...
                    if (GameCanvas.ispaintLogoGif)
                    {
                        ...
                        GameScr.paintGif(g, GameCanvas.hw - (imgW / 2), yPop - (imgH / 2) - 20);
                    }
                    else
                    {
                        
                        g.drawImageScale(ModFunc.imgLogoBig, GameCanvas.hw - (imgW / 2), yPop - (imgH / 2) - 20, imgW, imgH);
                    }
                }
            }
            else
            {
                ...
                if (GameCanvas.h > 160 && ModFunc.imgLogoBig != null)
                {
                    ...
                    if (GameCanvas.ispaintLogoGif)
                    {
                        imgW = ModFunc.logos[0].getWidth() * mGraphics.zoomLevel / 4;
                        imgH = ModFunc.logos[0].getHeight() * mGraphics.zoomLevel / 4;
                        GameScr.paintGif(g, GameCanvas.hw - (imgW / 2), num - (imgH / 2) - 20);
                    }
                    else
                    {
                        
                        g.drawImageScale(ModFunc.imgLogoBig, GameCanvas.hw - (imgW / 2), num - (imgH / 2), imgW, imgH);
                    }
                }
                int num2 = 4;
                int num3 = num2 * 32 + 23 + 33;
    ...
    public override void updateKey()
    {
        ...
        if (ModFunc.isOpenAccMAnager)
        {
            ...
            foreach (Command cmd in ModFunc.GI().cmdsDelAcc)
            {
                if (cmd.isPointerPressInside())
                {
                    cmd.performAction();
