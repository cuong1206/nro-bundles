...
public class mGraphics
{
    ...
    public void drawRegionm(Image arg0, int x0, int y0, int w0, int h0, int arg5, int x, int y, int arg8, int wnew, int hnew)
    {
        if (arg0 != null)
        {
            x *= zoomLevel;
            ...
            w0 *= zoomLevel;
            h0 *= zoomLevel;
            _drawRegionnew(arg0, x0, y0, w0, h0, arg5, x, y, arg8);
    ...
    public void ReleaseImage(ref Image image)
    {
        if (image != null && image.texture != null)
        {
            ...
            Debug.Log("🧹 Đã giải phóng Texture2D từ Image.");
        }
    }
    ...
    public void _drawRegionnew(Image image, float x0, float y0, int w, int h, int transform, int x, int y, int anchor)
    {
        if (image == null)
        {
            return;
        ...
        Graphics.DrawTexture(new Rect(((float)x + 450 + num3 + num14 + (float)num16)/2, (float)(y + num4 + (float)num17 + num15)/2, 
            (num * num7) / 2, (num2 * (float)num9)/2), image.texture, new Rect((x0 + num3 + num8) / (float)image.texture.width, ((float)image.texture.height - num2 - (y0 + num4)) / (float)image.texture.height, num / (float)image.texture.width, num2 / (float)image.texture.height), 0, 0, 0, 0);
        if (transform == 5 || transform == 6 || transform == 4 || transform == 7)
        {
            GUI.matrix = matrixBackup;
        }
    ...
    public void _drawRegionmob(Image image, float x0, float y0, int w, int h, int transform, int x, int y, int anchor, int wnew, int hnew)
    {
        if (image == null)
        {
            return;
        ...
        image = Image.createImage(image,
                                                ...
                                                (int)num2,
                                                0);
        drawImageScale(image, x,y, (int)num, (int)num2 , 0);
        if (transform == 5 || transform == 6 || transform == 4 || transform == 7)
        {
            GUI.matrix = matrixBackup;
        }
