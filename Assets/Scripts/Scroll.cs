public class Scroll
{
    ...
    public void setStyle(int[] itemHeights, int xPos, int yPos, int width, int height, bool styleUPDOWN)
    {
        this.xPos = xPos;
        this.yPos = yPos;
        this.width = width;
        ...
        // Sử dụng for-each loop trong C#
        foreach (int h in itemHeights)
        {
            totalHeight += h;
        }

        // Thiết lập giới hạn cuộn dọc
        if (styleUPDOWN)
        {
            cmyLim = totalHeight - height;
        }
        else
        {
            cmxLim = totalHeight - width;
        }
