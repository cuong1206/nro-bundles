	...
	public class BigBoss : Mob, IMapObject
	{
        ...
        public void getDataB2()
        {
            ...
            if (asset != null)
            {
                try
                {
                    data.readData2FromAsset(asset);
                    data.img = GameCanvas.loadImage($"/effectdata/{id}/img.png");
                }
                catch (System.Exception ex)
                {
                    Res.err("❌ Lỗi khi đọc effect data từ asset: " + ex.Message);
                    Service.gI().requestModTemplate(templateId);
                }
            }
            else
            {
                Debug.LogWarning($"⚠️ Không tìm thấy asset: {bundlePath}, gửi yêu cầu đến server.");
                Service.gI().requestModTemplate(templateId);
            }
