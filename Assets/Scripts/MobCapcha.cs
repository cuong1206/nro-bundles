public class MobCapcha
{
	...
	public static void paint(mGraphics g, int x, int y)
	{
		if (!isAttack)
		{
			...
			cmtoY = Char.myCharz().cy - 60 - GameScr.cmy;
		}
		else
		{
			delay++;
		...
		g.drawImage(GameScr.imgCapcha, cmx + 3, cmy - (GameScr.imgCapcha.getHeight() + 37), 3);
		PopUp.paintPopUp(g, cmx - 25, cmy - 70, mFont.tahoma_7b_dark.getWidthExactOf(GameScr.gI().keyInput) + 5, mFont.tahoma_7b_dark.getWidthExactOf(GameScr.gI().keyInput) / 2, 16777215, isButton: false);
		mFont.tahoma_7b_dark.drawString(g, GameScr.gI().keyInput, cmx - 20, cmy - 65, 0);
		if (isCreateMob)
		{
			isCreateMob = false;
