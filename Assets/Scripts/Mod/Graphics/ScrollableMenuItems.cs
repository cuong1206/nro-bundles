...
namespace Mod.Graphics
{
    internal class ScrollableMenuItems<T>
    {
        ...
        internal void UpdateKey()
        {
            ...
            if (GameCanvas.isPointerHoldIn(x, y, width, height))
            {
                if (!GameCanvas.isPointerMove || !GameCanvas.isPointerJustRelease)
                {
                    ...
                    if (GameCanvas.isPointerJustRelease && (lastMouseY == GameCanvas.pyMouse || lastMouseY == -1))
                    {
                        isPressKey = false;
        ...
        internal void Paint(mGraphics g)
        {
            ...
            for (int i = System.Math.Min((currentOffset + height) / itemHeight, items.Count - 1); i >= System.Math.Max(currentOffset / itemHeight, 0); i--)
            {
