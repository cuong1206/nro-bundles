...
namespace Mod.Graphics
{
    internal static class CustomGraphics
    {
        ...
        static void Initialize()
        {
            if (lineTex == null)
            {
                lineTex = new Texture2D(1, 1, TextureFormat.RGBA4444, false);
                lineTex.SetPixel(0, 1, Color.white);
                lineTex.Apply();
            }
            if (aaLineTex == null)
            {
                aaLineTex = new Texture2D(1, 3, TextureFormat.RGBA4444, false);
        ...
        //{
        //    int minSide = System.Math.Min(textureToCrop.width, textureToCrop.height);
        //    Texture2D texture2D = new Texture2D(minSide, minSide);
        //    for (int i = 0; i < minSide * minSide; i++)
        //    {
        //        int offset = System.Math.Abs(textureToCrop.width - textureToCrop.height) / 2;
        //        int x = i % minSide;
        //        int y = i / minSide;
