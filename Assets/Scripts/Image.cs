...
public class Image
{
    ...
    private static readonly string KEY = Main._636F6361696C6F6E6769616964632E2E();

    private static readonly string IV = Main._6B656F63616963633939313233332E2E();
    ...
    public static Image createImageNew(byte[] imageData)
    {
        return __createImagenew(imageData);
    }
    ...
    private static Image __createImagenew(byte[] encryptedImageData)
    {
        ...
        {
            // Giải mã dữ liệu ảnh
            byte[] decryptedImageData = DecryptImageData(encryptedImageData);

            image.texture.LoadImage(decryptedImageData);
    ...
    // Hàm mã hóa dữ liệu ảnh
    private static byte[] EncryptImageData(byte[] imageData)
    {
    ...
    private static byte[] DecryptImageData(byte[] encryptedData)
    {
        using (Aes aes = Aes.Create())
        {
            aes.Key = System.Text.Encoding.UTF8.GetBytes(KEY);
    ...
    private static Image __createImage2(string filename)
    {
        Image image = new Image();
        Texture2D texture2D = LoadAssetHelper.Load<Texture2D>(filename) as Texture2D;
        if (texture2D == null)
        {
            throw new Exception("NULL POINTER EXCEPTION AT Image __createImage " + filename);
        }
